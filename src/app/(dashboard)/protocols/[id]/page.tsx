  ...
  if (!protocol) {
    return (
      <div className="space-y-6">
        <Button variant="ghost" size="sm" asChild><Link href="/protocols"><ArrowLeft className="mr-2 h-4 w-4" />Quay lại</Link></Button>
        <div className="rounded-lg border border-dashed p-12 text-center"><p>Phác đồ không tồn tại</p></div>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        <Button variant="ghost" size="sm" asChild><Link href="/protocols"><ArrowLeft className="mr-2 h-4 w-4" />Quay lại</Link></Button>
        <div><h1 className="text-2xl font-bold">Chỉnh sửa phác đồ</h1></div>
      </div>

      <Card>
        <CardHeader><CardTitle>Thông tin phác đồ</CardTitle></CardHeader>
        <CardContent className="space-y-4">
          <div className="grid gap-4 md:grid-cols-2">
            <div className="space-y-2"><Label>Tên phác đồ</Label><Input value={name} onChange={(e) => setName(e.target.value)} /></div>
            <div className="space-y-2"><Label>Tình trạng mục tiêu</Label><Input value={targetCondition} onChange={(e) => setTargetCondition(e.target.value)} /></div>
          </div>
          <div className="space-y-2"><Label>Mô tả</Label><Textarea value={description} onChange={(e) => setDescription(e.target.value)} rows={3} /></div>
          <div className="grid gap-4 md:grid-cols-2">
            <div className="space-y-2"><Label>Thời gian (tuần)</Label><Input type="number" min={1} value={durationWeeks} onChange={(e) => setDurationWeeks(Number(e.target.value))} /></div>
            <div className="flex items-center space-x-2 pt-6">
              <Checkbox id="template" checked={isTemplate} onCheckedChange={(v) => setIsTemplate(v === true)} />
              <Label htmlFor="template">Lưu làm template</Label>
            </div>
          </div>
        </CardContent>
      </Card>
      ...
      <Card>
        <CardHeader>
          <div className="flex items-center justify-between">
            ...
            <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
              <DialogTrigger asChild><Button size="sm"><Plus className="mr-2 h-4 w-4" />Thêm bài tập</Button></DialogTrigger>
              <DialogContent className="max-h-[80vh] overflow-y-auto sm:max-w-lg">
                <DialogHeader><DialogTitle>Chọn bài tập</DialogTitle></DialogHeader>
                <Input placeholder="Tìm bài tập..." value={exSearch} onChange={(e) => setExSearch(e.target.value)} />
                  ...
                  {filteredCatalog.map((ex) => {
                    ...
                    return (
                      <div key={ex.id} className="flex items-center justify-between rounded-lg border p-3">
                        ...
                        <Button size="sm" variant={added ? "secondary" : "default"} disabled={added} onClick={() => addExercise(ex.id)}>{added ? "Đã thêm" : "Thêm"}</Button>
                      </div>
                    );
                  })}
                </div>
        ...
        <CardContent>
          {protoExercises.length === 0 ? (
            <div className="rounded-lg border border-dashed p-8 text-center text-muted-foreground">Chưa có bài tập. Nhấn &quot;Thêm bài tập&quot; để bắt đầu.</div>
          ) : (
            <Table>
              <TableHeader><TableRow><TableHead className="w-12">#</TableHead><TableHead>Bài tập</TableHead><TableHead>Sets</TableHead><TableHead>Reps</TableHead><TableHead>Thời gian</TableHead><TableHead>Tần suất</TableHead><TableHead className="w-16" /></TableRow></TableHeader>
              <TableBody>
      ...
      <div className="flex gap-3 justify-end">
        <Button variant="outline" asChild><Link href="/protocols">Hủy</Link></Button>
        <Button variant="secondary" onClick={() => alert("Gán cho bệnh nhân - coming soon")}>Gán cho bệnh nhân</Button>
        <Button onClick={() => alert("Đã lưu!")}><Save className="mr-2 h-4 w-4" />Lưu phác đồ</Button>
      </div>
    </div>
  );
}
